 ...
 * flat text format — header line, then number-first change rows
 * (`NN +content` / `NN -content`) with exact 1-based line numbers, context
 * rows (`old:new content`), and a ` ⋯` gap row between hunks. This module
 * renders ONLY the changed rows — removals red, additions green — and marks
 * every skipped run (context, hunk gaps) with a `…` ellipsis, so a big edit
 * stays compact. Pure DOM helpers, no framework imports — unit-testable in
 * jsdom.
 *
...
export function renderDiffRows(headline: HTMLElement): void {
  ...
  // A change row is `NN +content` / `NN -content`. The `old:new` colon on
  // context rows keeps them from matching, so a context line whose text
  // starts with `-` is never misread as a removal.
  let lastRendered = 0
